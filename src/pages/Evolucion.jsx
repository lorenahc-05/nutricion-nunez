...
import { PERFIL, REGISTROS_PESO } from '../data/menus'
import BottomNav from '../components/BottomNav'
import './Evolucion.css'

export default function Evolucion() {
  const [registros, setRegistros] = useState(REGISTROS_PESO)
  ...
  function handleGuardar() {
    ...
    setRegistros(prev => [...prev, { id: Date.now(), peso: val, fecha: today }])
    setNuevoP('')
  }
  ...
  const restante = ultimo ? (ultimo - PERFIL.peso_objetivo).toFixed(1) : null
    ...
    <div className="evolucion-page">
      ...
      <div className="stats-row">
        ...
        <div className="stat-card">
          ...
          <span className="stat-value">{PERFIL.peso_objetivo} kg</span>
          {restante && (
      ...
      <div className="evolucion-content">
          ...
          <div className="chart-card">
            ...
            <ResponsiveContainer width="100%" height={160}>
              <LineChart data={chartData}>
                ...
                <YAxis domain={['auto', 'auto']} tick={{ fontSize: 10, fill: '#aaa' }} axisLine={false} tickLine={false} tickFormatter={v => `${v}kg`} width={40} />
                <Tooltip formatter={v => [`${v} kg`, 'Peso']} contentStyle={{ fontSize: 12, borderRadius: 8, border: '1px solid #ede8e1' }} />
                <ReferenceLine y={PERFIL.peso_objetivo} stroke="#e8824a" strokeDasharray="4 4" />
                <Line type="monotone" dataKey="peso" stroke="#2d5a3d" strokeWidth={2} dot={{ fill: '#e8824a', r: 4, strokeWidth: 2, stroke: '#fff' }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        )}
        ...
        <div className="registro-card">
          ...
          <div className="registro-row">
            <input type="number" className="peso-input" value={nuevoP} onChange={e => setNuevoP(e.target.value)} placeholder="00.0" step="0.1" min="30" max="300" />
          ...
          {[...registros].reverse().map((r, i, arr) => {
            ...
            return (
              <div key={r.id} className="historial-row">
                <span className="historial-fecha">{new Date(r.fecha).toLocaleDateString('es-ES', { day: 'numeric', month: 'short' })}</span>
                <span className="historial-peso">{r.peso} kg</span>
                {diff && <span className={`historial-delta ${parseFloat(diff) < 0 ? 'down' : 'up'}`}>{parseFloat(diff) < 0 ? '▼' : '▲'} {Math.abs(diff)} kg</span>}
              </div>
            )
          })}
        </div>
